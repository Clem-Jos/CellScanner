    ...
    if 'Accuri' in fc or 'accuri' in fc:
        ...
        for row in list(narray.index):
            ...
            #  labels.append(0)
                ...
                labels.append(1)
    elif 'FSC-H' in list(narray.columns):
        for row in list(narray.index):
            if math.isnan(np.log10(narray.loc[row, 'FL3-A'])) or math.isnan(np.log10(narray.loc[row, 'FL1-A'])) \
                    or np.log10(narray.loc[row, 'FSC-A']) > (np.log10(narray.loc[row, 'FSC-H']) + 0.5) \
                    or np.log10(narray.loc[row, 'FSC-A']) < (np.log10(narray.loc[row, 'FSC-H']) -0.5)\
...
def neuralNetwork(data, target, ratio, activation='relu', solver='lbfgs', max_iter=200, random_state=None,rept=0):
    ...
    #     exp = explainer.explain_instance(test_img[i], grid.predict_proba, num_features=13, top_labels=2)
    #     a=exp.as_list()
    #     t = []
    #     v = []
    #     vsorted=[]
    ...
    #             if f in aval[0]:
    #                 t.append(f)
    #         v.append(str(abs(aval[1])))
    #     f = open('C:/Users/u0128864/Documents/KU_LEUVEN\FLOWCYTOMETRY/features.csv', 'a')
    #     tsorted = t.copy()
    #     tsorted.sort()
    #     for val in tsorted:
    #         vsorted.append(v[t.index(val)])
    #     t = species + tsorted
    #     v = species + vsorted
    #     f.write(sep.join(t) + '\n' + sep.join(v) + '\n')
    #     f.close()
    ...
    for df in statValues:
        ...
        for sp in species:
            if df.loc[sp, 'TP'] != 0:
                ...
                prec.append('None')
            if df.loc[sp, 'TN'] != 0:
                spec.append(df.loc[sp, 'TN'] / (df.loc[sp, 'TN'] + df.loc[sp, 'FP']))
            else:
                spec.append('None')
...
def machineGating(refArrays, species, predArrays, species2, predType='neur', ratio=1 / 2, random_state=None,
    ...
    (RefArrays). A prediction of each monoculture dataarray, will indicate, which lie is from line or from blanck.
    In fact, if the cells monoculture contains blank values, the prediction model will hesitate between blanck and
    ...
    :param param: list of 3 names of channels used for the 3D graph
    ...
    if 'Blank' in species or 'blank' in species or 'BLANK' in species:
        ...
        for aSp in list(set(sp2)):
            ...
            graph3d(data2, prediction, target2, species + ['unknown'], name=nName, show=show, cwd=cwd, param=param)
    ...
    if repeat > 1:
        for i in range(len(predict_lbls[0])):
            ...
            if l.count(v) <= repeatm:
                ...
                sp = list(set(l))
                cnt = [l.count(c) for c in sp]
                res = []
                for i in range(len(cnt)):
                    if cnt[i] == l.count(v):
                        res.append(sp[i])
                if len(res) == 1:
                    predict_lbl.append(res[0])
                else:
                    predict_lbl.append(rand.choice(res))
            # if l.count(v) > repeatm:
            #     predict_lbl.append(v)
            #
            # elif l.count(v)==repeat/2 and repeat/2 > repeatm:# fonctionne pour 2 mais pas pour plus:
            #     #print(rand.choice)
            #     predict_lbl.append(rand.choice(l))
            #
            # else:
            #     predict_lbl.append('unknown')
        return predict_lbl
